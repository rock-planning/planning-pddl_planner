...
PlanCandidates Planner::plan(const std::string& problem, const std::string& actionDescriptions, const std::string& domainDescriptions)
{
    ...
    int result = system("which tfd-planner");
    if(result != 0)
    {
        std::string msg = "Could not find 'tfd-planner' binary";
        LOG_ERROR("%s",msg.c_str());
        throw PlanGenerationException(msg);
    }

    std::string currentTime = base::Time::now().toString();
    fs::path path(msTempDirBasename + "/" + currentTime + "_tfd");
...
{
    std::string cmd = "tfd-planner " + mDomainFilename + " " + mProblemFilename + " " + mResultFilename;
...
Plan Planner::readPlan(const std::string& filename)
{
    ...
    {
        char buffer[512];
        snprintf(buffer, 512, "TFD: could not open '%s'", filename.c_str());
        LOG_ERROR("%s", buffer);
